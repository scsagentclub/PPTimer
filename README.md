...
6. 按 `Esc` 退出；在窗口空白处、时间或 logo 上右键可选择「退出 Exit」。  
   Press `Esc` to exit, or right-click on an empty area, the time, or the logo to select "Exit".
...
8. 点击 `T` 按钮可切换主题（深色 / 浅色 / 蓝色 / 紫色），主题偏好会自动保存。  
